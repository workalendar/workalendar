...
from ..core import MON, SAT, SUN, Calendar, ChristianMixin, WesternCalendar
...
class Canada(WesternCalendar, ChristianMixin):
    "Canada"
    FIXED_HOLIDAYS = WesternCalendar.FIXED_HOLIDAYS + ((7, 1, "Canada Day"),)
    shift_new_years_day = True

    def get_variable_days(self, year):
        # usual variable days
        days = super(Canada, self).get_variable_days(year)
        days.append((Canada.get_nth_weekday_in_month(year, 9, MON, 1), "Labor Day"))
        # Canada day
...
class StJeanBaptisteMixin(Calendar):
    ...
    def get_st_jean(self, year):
        ...
        if stjean.weekday() in self.get_weekend_days():
            days = [
                (stjean, "St Jean Baptiste"),
                (self.find_following_working_day(stjean), "St Jean Baptiste (Shift)"),
            ]
        else:
...
class Quebec(Canada, VictoriaDayMixin, StJeanBaptisteMixin, ThanksgivingMixin):
    ...
    def get_variable_days(self, year):
        ...
        days.extend([(self.get_victoria_day(year)), (self.get_thanksgiving(year))])
...
class BritishColumbia(
    ...
    FIXED_HOLIDAYS = Canada.FIXED_HOLIDAYS + ((11, 11, "Remembrance Day"),)
...
class NorthwestTerritories(
    Canada, RemembranceDayShiftMixin, VictoriaDayMixin, ThanksgivingMixin
):
    "Northwest Territories"

    FIXED_HOLIDAYS = Canada.FIXED_HOLIDAYS + ((6, 21, "National Aboriginal Day"),)
...
@iso_register("CA-NU")
class Nunavut(Canada, VictoriaDayMixin, ThanksgivingMixin, RemembranceDayShiftMixin):
    "Nunavut"
    include_good_friday = True
