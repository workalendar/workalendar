...
from ..core import MON, ChristianMixin, WesternCalendar
...
class Ireland(WesternCalendar, ChristianMixin):
    ...
    def get_june_holiday(self, year):
        return (Ireland.get_nth_weekday_in_month(year, 6, MON), "June Holiday")

    def get_august_holiday(self, year):
        return (Ireland.get_nth_weekday_in_month(year, 8, MON), "August Holiday")
    ...
    def get_variable_days(self, year):
        ...
        # May Day
        if year >= 1994:
            days.append((Ireland.get_nth_weekday_in_month(year, 5, MON), "May Day"))
        ...
        if year >= 1977:
            days.append(
                (Ireland.get_last_weekday_in_month(year, 10, MON), "October Holiday")
            )
